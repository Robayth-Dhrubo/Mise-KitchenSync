...
function StatSkeleton() {
    return <div className="h-8 w-12 bg-zinc-800 rounded animate-pulse" />
}
    ...
    useEffect(() => {
        ...
        const fetchAllData = async () => {
            try {
                ...
                // Filter inventory alerts if not admin/owner
                ...
                // Add operations-only fetches (margin alerts, sales, recipes)
                if (['admin', 'owner', 'chef'].includes(role)) {
                    ...
                    // Only filter if NOT admin/owner/chef
                    if (!['admin', 'owner', 'chef'].includes(role)) {
                ...
                if (role === 'foh') {
                    ...
                    fetchPromises.push(
                        // Occupied Tables
                        supabase.from('locations').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('status', 'occupied') as unknown as Promise<any>,
                        // Today's Reservations
                        supabase.from('reservations').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('reservation_date', today) as unknown as Promise<any>
                    )
                }
                    ...
                    <div className="bg-zinc-900/10 border border-white/5 rounded-2xl p-6 flex flex-wrap gap-4 items-center">
                        ...
                        {isOperations && (
                            <Link href="/kitchen-manager" className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 transition-all text-xs font-bold text-zinc-300">
                                <ChefHat className="w-4 h-4 text-emerald-500" /> Kitchen
                            </Link>
                        )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        {[
                                            ...
                                            <div key={label} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-neutral-300 flex items-center gap-2 text-sm font-bold uppercase tracking-wider">
                                                        <Icon className="w-4 h-4" /> {label}
                                                    </span>
                                                </div>
                                                <span className="text-emerald-500 text-[10px] font-black">{status}</span>
                                            </div>
                                        ))}
                                    </CardContent>
                                </Card>
                                ...
                                <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div>
                                            ...
                                            <div className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Open Tickets</div>
                                        </div>
                                        <div>
                                            <div className="text-3xl font-black text-white mb-1 tabular-nums">
                                                {isLoading ? <StatSkeleton /> : stats.activeStaff}
                                            </div>
                                            <div className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Active Staff</div>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {isOperations && (
                                <div className="space-y-4">
                                    <Link href="/finance/margin-guard" className="block group">
                                        <div className={cn(
                                            "bg-zinc-900/50 border rounded-2xl p-6 transition-all duration-500",
                                            marginAlerts.length > 0
                                                ? "border-amber-500/50 hover:bg-amber-500/5 hover:border-amber-500 shadow-lg shadow-amber-500/10"
                                                : "border-zinc-800/50 hover:border-zinc-700 hover:bg-zinc-800/30"
                                        )}>
                                    ...
                                    {marginAlerts.length > 0 && (
                                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-700">
                                            <div className="flex items-center justify-between px-2">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Active Alerts</span>
                                                <RefreshCw className={cn("w-3 h-3 text-zinc-700", isLoading && "animate-spin")} />
                                            </div>
                                            {marginAlerts.slice(0, 3).map((alert) => {
                                                const foodCostPct = ((alert.new_cost / alert.current_menu_price) * 100).toFixed(1)
                                                return (
                                                    <div key={alert.id} className="bg-zinc-900/80 border border-zinc-800/50 rounded-xl p-4 hover:border-amber-500/30 transition-all">
                                                        <div className="flex items-start justify-between gap-4 mb-3">
                                                            <div className="flex items-center gap-2">
                                                                <div className="w-6 h-6 rounded bg-zinc-800 flex items-center justify-center border border-zinc-700">
                                                                    <ChefHat className="w-3 h-3 text-zinc-500" />
                                                                </div>
                                                                <div>
                                                                    <p className="text-xs font-bold text-white leading-none">{alert.recipe.name}</p>
                                                                    <p className="text-[10px] text-amber-500 font-bold uppercase mt-0.5">{alert.ingredient.name} hike</p>
                                                                </div>
                                                            </div>
                                                            <div className="text-right">
                                                                <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest leading-none mb-1">Food Cost</p>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <Button
                                                                size="sm"
                                                                className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-[10px] h-8 font-black uppercase tracking-tight text-white"
                                                            ...
                                                            <Button
                                                                size="sm"
                                                                variant="ghost"
                                                                className="px-2 h-8 text-zinc-500 hover:text-white"
                                            ...
                                            {marginAlerts.length > 3 && (
                                                <Link href="/finance/margin-guard" className="block text-center py-2 text-[10px] font-black uppercase tracking-widest text-zinc-600 hover:text-white transition-colors">
                            ...
                            {isOperations && !isLoading && (
                                <div className="pt-8 border-t border-white/5">
                                    <div className="flex items-center gap-3 mb-8">
                                        ...
                                        <h2 className="text-xl font-black text-white uppercase tracking-tight">Financial Analytics</h2>
                                    </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-neutral-900/30 border border-neutral-800/50 rounded-2xl p-6">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    <Link href="/kitchen-manager" className="block p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all border border-white/5 group">
                                        ...
                                        <div className="text-[10px] text-neutral-500 font-bold">Manage orders & menu</div>
                                    </Link>
                                </div>
                            </div>
                            ...
                            {(isOperations || userRole === 'chef') && (
                                <Card className={cn(
                                    "bg-neutral-900/50 border-white/5 transition-all duration-500 overflow-hidden",
                                    soldOutItems.length > 0 && "border-red-500/20 ring-1 ring-red-500/10"
                                )}>
                                    <CardHeader className="pb-4 border-b border-white/5 bg-white/[0.02]">
                                        <div className="flex items-center justify-between">
                                            <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 flex items-center gap-2">
                                                <UtensilsCrossed className={cn("w-4 h-4", soldOutItems.length > 0 ? "text-red-500" : "text-emerald-500")} />
                                                86'd Board
                                            </CardTitle>
                                            <Badge variant="outline" className={cn(
                                                "font-mono text-[9px] border-white/10",
                                                soldOutItems.length > 0 ? "text-red-500" : "text-emerald-500"
                                            )}>
                                    ...
                                    <CardContent className="p-0">
                                        {isLoading ? (
                                            ...
                                            <div className="p-8 text-center">
                                                ...
                                                <p className="text-[10px] text-zinc-600 font-black uppercase tracking-widest italic">All Dishes Available</p>
                                            </div>
                                        ) : (
                                            <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto no-scrollbar">
                                                {soldOutItems.map((item) => (
                                                    <div key={item.id} className="p-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-8 h-8 rounded-lg overflow-hidden bg-zinc-800 border border-white/10 grayscale opacity-40">
                                                                {item.image_url ? (
                                                            ...
                                                            <div>
                                                                <div className="text-[11px] font-bold text-zinc-400 line-through decoration-red-500/50">{item.name}</div>
