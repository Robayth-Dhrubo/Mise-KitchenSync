        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 mt-1">Manage staff, roles, and access permissions.</p>
                </div>
                <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={fetchData} size="sm" className="border-white/10 hover:bg-white/5 text-neutral-300">
                        ...
                        <DialogContent className="bg-neutral-900 border-white/10 text-white sm:max-w-md">
                            ...
                            <div className="space-y-4 mt-4">
                                <div className="space-y-2">
                                    <Label className="text-xs text-neutral-400">EMAIL ADDRESSES</Label>
                                    <Textarea
                                    ...
                                    />
                                    <p className="text-[10px] text-neutral-500">Separate multiple emails with commas or new lines.</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs text-neutral-400">ASSIGN ROLE</Label>
                                ...
                                <Button onClick={handleInvite} disabled={isInviting || !emailList} className="w-full bg-emerald-600 text-white hover:bg-emerald-500">
                                    {isInviting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Send Invites'}
                                </Button>
                                {inviteResult && (
                                    <div className={`text-xs p-3 rounded-lg border ${inviteResult.results?.some((r: any) => r.success) ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                                            ...
                                            <TableRow key={u.id} className="border-white/5 hover:bg-white/5">
                                                <TableCell className="pl-6 font-medium">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-neutral-800 to-neutral-700 flex items-center justify-center text-xs font-bold text-neutral-300 border border-white/10">
                                                            {u.email.slice(0, 2).toUpperCase()}
                                                        </div>
                                                        <div>
                                                            <div className="text-neutral-200">{u.email}</div>
                                                            {u.restaurant_name && <div className="text-xs text-neutral-500">{u.restaurant_name}</div>}
                                                        </div>
                                                    </div>
                                                </TableCell>
                                                <TableCell>
                                                ...
                                                <TableCell>
                                                    {u.email_confirmed_at ?
                                                        <Badge variant="outline" className="text-emerald-500 border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10">Verified</Badge>
                                                ...
                                                <TableCell className="text-right pr-6">
                                                    {!u.email_confirmed_at && (
                                                        <Button size="sm" className="h-7 text-xs bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/40 border border-emerald-500/20" onClick={() => handleConfirmUser(u.id)}>
