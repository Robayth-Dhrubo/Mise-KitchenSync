...
export function ReportsDashboard({ salesLogs, recipes }: ReportsDashboardProps) {
    ...
    const stats = useMemo(() => {
        ...
        // Menu Matrix Calculation
        const matrixData = recipes.map(recipe => {
            const sales = salesByRecipe[recipe.id]?.quantity || 0

            // Calculate Cost using helper
            // Transform recipe.recipe_items to match RecipeItemWithIngredient interface if needed
            // Currently recipe.recipe_items has `ingredient` populated from Supabase query
            const costCalc = calculateRecipeCost(recipe.recipe_items || [], recipe.menu_price || 0)
            const margin = costCalc.gross_margin_dollars
            ...
            return {
                ...
                category: 'Dog' // Placeholder, calculated below
            }
        }).filter(item => item.popularity > 0 || item.profitability > 0) // Filter out inactive items if desired

        // Calculate Averages for Quadrants
        const avgPop = matrixData.reduce((sum, item) => sum + item.popularity, 0) / (matrixData.length || 1)
        const avgProf = matrixData.reduce((sum, item) => sum + item.profitability, 0) / (matrixData.length || 1)

        // Assign Categories
        const labeledMatrixData = matrixData.map(item => {
            let category: 'Star' | 'Plowhorse' | 'Puzzle' | 'Dog'
            if (item.popularity >= avgPop && item.profitability >= avgProf) category = 'Star'
            else if (item.popularity >= avgPop && item.profitability < avgProf) category = 'Plowhorse'
            else if (item.popularity < avgPop && item.profitability >= avgProf) category = 'Puzzle'
            else category = 'Dog'
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            {/* Menu Matrix Chart */}
            <MenuMatrixChart data={stats.matrixData} />
