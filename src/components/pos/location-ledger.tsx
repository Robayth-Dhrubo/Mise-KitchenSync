...
export default function LocationLedger({ location, onClose, onOpenTerminal }: LocationLedgerProps) {
    ...
    return (
        <div className="absolute inset-y-0 right-0 w-[440px] bg-black/90 backdrop-blur-3xl border-l border-white/10 shadow-4xl z-50 flex flex-col animate-in slide-in-from-right duration-500">
            {/* Header - Minimal & Large */}
            <div className="p-8 pb-4 flex flex-col gap-1 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8">
                    <Button variant="ghost" size="icon" onClick={onClose} className="w-10 h-10 rounded-full hover:bg-white/10 text-neutral-500 hover:text-white transition-colors">
                        <X className="w-5 h-5" />
                    </Button>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <Badge variant="outline" className={cn(
                        ...
                        location.status === 'available' ? "bg-emerald-500/20 text-emerald-500" :
                ...
                <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest">{location.type} • ID: {location.id.slice(0, 4)}</p>
            ...
            <div className="grid grid-cols-2 gap-4 px-8 py-4">
                <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex flex-col gap-1 relative overflow-hidden group">
                    ...
                    <span className="text-[9px] font-black text-neutral-500 uppercase tracking-widest z-10">Current Tab</span>
                    <span className="text-2xl font-black text-emerald-500 tracking-tighter z-10">${totalDues.toFixed(2)}</span>
                </div>
                <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex flex-col gap-1 relative overflow-hidden group">
                    <div className="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <span className="text-[9px] font-black text-neutral-500 uppercase tracking-widest z-10">Status</span>
                    <span className="text-xl font-black text-white tracking-tight uppercase z-10">{activeOrder ? 'Active' : 'Idle'}</span>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-black/20">
                <div className="px-8 pt-6 pb-2">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Recent Transactions</h3>
                </div>
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <Badge className={cn(
                                                    ...
                                                    order.preparation_status === 'firing' ? "bg-amber-500 text-black" :
                                                        order.preparation_status === 'completed' ? "bg-neutral-800 text-neutral-400" :
                                            ...
                                            <div className="text-xs font-bold text-neutral-400">Order #{order.id.slice(0, 5)}</div>
                                        </div>
                                        <div className="text-lg font-black text-white">${order.total_amount.toFixed(2)}</div>
                                    </div>
                                ))
                            )}
                        </div>
            ...
            <div className="p-6 pt-2">
                <div className="grid grid-cols-5 gap-3">
                    <Button
                        variant="outline"
                        className="col-span-2 h-14 rounded-2xl border-white/10 bg-white/5 text-white hover:bg-white/10 text-[9px] font-black uppercase tracking-widest transition-all"
                    >
                        Settle
                    </Button>
                    <Button
                        onClick={onOpenTerminal}
                        className="col-span-3 h-14 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-900/20 transition-all gap-2"
                    >
